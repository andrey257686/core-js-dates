...
function getDayName(date) {
  ...
  return daysArr[new Date(date).getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  const offset = 5 - date.getUTCDay();
  if (offset > 0) {
    nextFriday.setUTCDate(date.getUTCDate() + offset);
  } else {
    nextFriday.setUTCDate(date.getUTCDate() + 7 + offset);
  }
  return nextFriday;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysOffset = Math.floor((date - firstYearDay) / (24 * 60 * 60 * 1000));
  const firstMondayOffset = (firstYearDay.getDay() + 6) % 7;
  const result = Math.floor((daysOffset + firstMondayOffset) / 7) + 1;
  return result;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      workSchedule.push(currentDate.toLocaleDateString('es-CL'));
